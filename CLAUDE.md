...
- **Input'lar**: yumuşak border, geniş radius, focus'ta accent border + `--pt-accent-ring` box-shadow. Arama kutuları pill.
...
- Ayrıca: react-select seçili/focused option ve chip'leri, date-picker seçili gün + bugün vurgusu, tab aktif underline, radio checked, link renkleri, breadcrumb aktif öğe, drawer/modal primary aksiyonları, progress/loading bar, version badge'leri.
...
- **Nav**: ikonlu, gruplu custom sidebar. İkonlar plugin option'ından gelir (`nav.icons`), lucide-react kullanılır. Collection grupları desteklenir.
- **Logo/Icon**: plugin option'ı ile URL veya component path kabul eder.
...
- Component export'ları Payload 3 import map kurallarına uygun string path formatında: `'@liderbektas/payload-theme/client#X'` / `'/rsc#Y'`. `payload generate:importmap` sonrası sorunsuz resolve olmalı.
...
**Faz 5 — Replacement layer:** Nav swap (ikon sistemi dahil), Logo/Icon, en son Dashboard.
